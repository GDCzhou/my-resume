...
description: "16. 如何设计多 Agent 的协作与动态切换机制？ 👔面试官：多 Agent 系统里，各个 Agent 之间怎么协作？ 🙋‍♂️我：一个 Agent 做完之后把结果传给下一个 Agent，就像流水线一样，一步步往下走。 👔面试官：你说的…"
---

# 16. 如何设计多 Agent 的协作与动态切换机制？

<div class="qa-dialog">
<p class="qa-q"><strong>面试官</strong>：多 Agent 系统里，各个 Agent 之间怎么协作？</p>

<p class="qa-a"><strong>我</strong>：一个 Agent 做完之后把结果传给下一个 Agent，就像流水线一样，一步步往下走。</p>

<p class="qa-q"><strong>面试官</strong>：你说的是流水线，但「传结果」具体怎么传？消息传递和共享状态是两种不同的方案，适用场景也不一样，你区分得开吗？</p>

<p class="qa-a"><strong>我</strong>：消息传递就是 Agent 之间直接发消息，共享状态就是大家都能读写同一个变量……应该都差不多吧？</p>

<p class="qa-q"><strong>面试官</strong>：差很远。消息传递的核心优势是解耦，发送方不需要知道谁在接收；共享状态的优势是直接，前一步写进去后一步直接读。这两种选哪个，取决于 Agent 之间的依赖关系强不强。那动态切换呢，你是怎么做的？</p>

<p class="qa-a"><strong>我</strong>：动态切换就是让 LLM 判断下一步该调用哪个 Agent，每次根据当前情况动态决策，这样最灵活。</p>

<p class="qa-q"><strong>面试官</strong>：全靠 LLM 动态决策的问题是什么？每次路由都要多一次 LLM 调用，而且 LLM 偶尔会路由错，系统行为的可预测性就没了。你有没有想过，静态路由和动态路由应该怎么配合用？</p>
...
![](/images/cdn.xiaolincoding.com/picgo/01_collaboration_modes.png)
...
![](/images/cdn.xiaolincoding.com/picgo/02_message_vs_shared_state.png)
...
![](/images/cdn.xiaolincoding.com/picgo/03_shared_state_layers.png)
...
![](/images/cdn.xiaolincoding.com/picgo/04_static_vs_dynamic_routing.png)

两种路由策略的对比可以用一张图来理解：

![](/images/cdn.xiaolincoding.com/picgo/image-20260305214923533.png)
...
![](/images/cdn.xiaolincoding.com/picgo/05_handoff_relay.png)
...
![](/images/cdn.xiaolincoding.com/picgo/06_hybrid_routing.png)
